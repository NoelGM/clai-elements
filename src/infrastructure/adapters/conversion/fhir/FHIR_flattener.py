...
def find_patient(bundle):
    ...
    if len(patients) < 1:
        ...
        # Acceso seguro a name/given/family
        name_obj = patient.get('name', [{}])
        if name_obj and isinstance(name_obj, list) and len(name_obj) > 0:
            given = name_obj[0].get('given', [''])
            first_name = given[0] if given else ''
            last_name = name_obj[0].get('family', '')
        else:
            first_name = ''
            last_name = ''
