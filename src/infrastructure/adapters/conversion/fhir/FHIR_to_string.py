...
def any_date_to_str(fhir_date, converter, parent_field=''):
    ...
    try:
        ...
        return [f'This {converter.doc} was {what} on {data_time[0]}-{data_time[1]}-{data_time[2]} at {data_time[3]}.'] #Formateo de la fecha para que sea formato ISO 8601
    except IndexError:
        data_time = date_time_pattern2.findall(fhir_date)[0]
        return [f'This {converter.doc} was {what} on {data_time[0]}/{data_time[1]}/{data_time[2]}']
...
def any_value_quantity_to_str(fhir_value, converter, parent_field=''):
    value = fhir_value.get("value", "unknown")
    unit = fhir_value.get("unit", "")  # Evita el KeyError si "unit" no está presente
    what = parent_field_to_str(parent_field, 'value')
    
    if unit:
        return [f'The {what} for this {converter.doc} is {value} {unit}.']
    else:
        return [f'The {what} for this {converter.doc} is {value}.']
